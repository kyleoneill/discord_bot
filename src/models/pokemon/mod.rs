...
impl PokemonData {
    pub fn new() -> Self {
        ...
        let contents = fs::read_to_string("data/pokedex_gold.json")
            .expect("Failed to read pokemon_data.json file");
        ...
        match v {
            Value::Object(pokemon_map) => {
                for key in pokemon_map.keys() {
                    let entry = pokemon_map
                        .get(key)
                        .expect("Failed to get a pokemon with a known key");
                    let mut parsed_pokemon: Pokemon = serde_json::from_value(entry.to_owned())
                        .expect("Failed to parse pokemon entry from JSON");
    ...
    pub fn get_random_pokemon(&self) -> Pokemon {
        ...
        let (_pokemon_name, pokemon) = self
            .data
            .get_index(index)
            .expect("Failed to get a random pokemon");
        pokemon.clone()
    }
}
